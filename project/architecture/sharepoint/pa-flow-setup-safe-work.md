# Power Automate Flow Setup: Safe Work

Step-by-step guide to create the Safe Work flow in Power Automate.

---

## Step 1: SharePoint List "Safe Work Permits"

List already exists with PascalCase internal names (e.g. `CompanyPerson`). Verify these columns exist:
...
| CompanyPerson | Company Person | Single line of text | |
...
| RequestedBy | Requested By | Single line of text | |
...
```json
{
  ...
  "data": {
    ...
    "WorkScope": "Inspect valve seals",
    "SpecialInstructions": "Wear PPE at all times",
    "RequestedBy": "DK",
    "Status": "Active"
  },
  "attachments": [
...
| Date | `item()?['Date']` |
| Time | `item()?['Time']` |
| CompanyPerson | `item()?['CompanyPerson']` |
...
- **Java:** `src/main/resources/application.properties` > `pa.flow.safe-work-url`
...
curl -X POST "<your-flow-url>" \
  ...
  -d '{
    ...
    "data": {
      ...
      "Date": "2026-03-03",
      "Time": "08:00",
      "CompanyPerson": "Test User",
      "LocationOfWork": "Turbine Hall",
      "WorkScope": "Test safe work scope",
      "SpecialInstructions": "Wear PPE at all times",
...
2. Open the app, go to Safe Work Permits
3. Fill out a form and submit
4. Check the browser console for `[PA V2] Submitting safeWork: create`
...
2. **Date and Time as strings.** The Date and Time columns are stored as Single line of text, not SharePoint date columns. This avoids timezone conversion issues. Send them as plain strings (e.g. `"2026-03-03"`, `"08:00"`).
