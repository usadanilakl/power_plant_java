# Power Automate Flow Setup: Confined Space

Step-by-step guide to create the Confined Space flow in Power Automate.

---

## Step 1: SharePoint List "Confined Space Permits"

List already exists with PascalCase internal names (e.g. `SpaceToBeEntered`). Verify these columns exist:
...
| Duration | Duration | Single line of text | |
| MeterModel | Meter Model | Single line of text | |
| MeterNum | Meter Number | Single line of text | |
| Calibrated | Calibrated | Yes/No | Default: No |
...
```json
{
  ...
  "data": {
    ...
    "Title": "Boiler inspection entry",
    "Date": "2026-03-03",
    "Time": "08:00",
    "SpaceToBeEntered": "Boiler Room 2",
    "IssuedTo": "DK",
    "Duration": "4 hours",
    "MeterModel": "GX-2009",
    "MeterNum": "M-4521",
...
3. List Name: **Confined Space Permits**
...
| Duration | `triggerBody()?['data']?['Duration']` |
| Meter Model | `triggerBody()?['data']?['MeterModel']` |
| Meter Number | `triggerBody()?['data']?['MeterNum']` |
| Calibrated | `triggerBody()?['data']?['Calibrated']` |
| Status | `triggerBody()?['data']?['Status']` |

**Tip:** For Choice columns (Status), the value must match one of the defined choices exactly (e.g. `Active`, `Closed`, `Archived`). For the Yes/No column (Calibrated), send a boolean value (`true` or `false`).
...
2. If **Yes**:
   ...
   - Inside the loop, add **SharePoint > Add attachment**:
     ...
     - List Name: **Confined Space Permits**
...
**Note:** The Status column is a Choice and returns an object like `{"Value": "Active"}`. Use `?['Value']` to extract the string. The Calibrated column is a Yes/No column and returns a boolean directly (no `?['Value']` needed).
...
- Full update: `{ "actionType": "update", "id": "42", "data": { "SpaceToBeEntered": "Boiler Room 3", "Status": "Active" } }`
...
- **Java:** `src/main/resources/application.properties` > `pa.flow.confined-space-url`
...
2. Open the app, go to Confined Space Permits
3. Fill out a form and submit
4. Check the browser console for `[PA V2] Submitting confinedSpace: create`
...
2. **Yes/No column (Calibrated).** Send a boolean value (`true` or `false`), not a string. In the sample JSON payload, `"Calibrated": true` is correct. Sending `"Calibrated": "true"` (as a string) may cause unexpected behavior.
...
9. **Calibrated in getAll Select.** The Calibrated column is a Yes/No type and returns a boolean directly (`true`/`false`). Do NOT use `?['Value']` on it — that pattern is only for Choice columns like Status.
