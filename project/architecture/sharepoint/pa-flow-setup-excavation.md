# Power Automate Flow Setup: Excavation Permit

Step-by-step guide to create the Excavation Permit flow in Power Automate.

---

## Step 1: SharePoint List "Excavation Permits"

List already exists with PascalCase internal names (e.g. `ExcavationDescription`). Verify these columns exist:
...
| Supervisor | Supervisor | Single line of text | |
| JobLocation | Job Location | Single line of text | |
| SupervisorPhone | Supervisor Phone | Single line of text | |
| ExcavationDescription | Excavation Description | Multiple lines of text | Plain text |
| WorkOrder | Work Order | Single line of text | |
| LocationPipingMarked | Location/Piping Marked | Yes/No | Default: No |
| FacilityName | Facility Name | Single line of text | |
| CompetentPerson | Competent Person | Single line of text | |
| SoilType | Soil Type | Single line of text | |
| ExcavationDepth | Excavation Depth | Single line of text | |
| ExcavationWidth | Excavation Width | Single line of text | |
| ProtectiveSystemType | Protective System Type | Single line of text | |
...
```json
{
  ...
  "data": {
    ...
    "Date": "2026-03-03",
    "Time": "07:00",
    "LocationOfWork": "South Yard",
    "IssuedTo": "JD",
    "Supervisor": "MR",
    "JobLocation": "South Yard - Pipe Trench Area",
    "SupervisorPhone": "555-1234",
    "ExcavationDescription": "Trench excavation for new cooling water piping",
    "WorkOrder": "WO-2026-0302",
    "LocationPipingMarked": true,
    "FacilityName": "Main Plant",
    "CompetentPerson": "DK",
    "SoilType": "Type B",
    "ExcavationDepth": "6 ft",
    "ExcavationWidth": "4 ft",
    "ProtectiveSystemType": "Trench Box",
    "Status": "Active"
  },
  "attachments": [
    {
...
3. List Name: **Excavation Permits**
...
| Supervisor | `triggerBody()?['data']?['Supervisor']` |
| Job Location | `triggerBody()?['data']?['JobLocation']` |
| Supervisor Phone | `triggerBody()?['data']?['SupervisorPhone']` |
| Excavation Description | `triggerBody()?['data']?['ExcavationDescription']` |
| Work Order | `triggerBody()?['data']?['WorkOrder']` |
| Location/Piping Marked | `triggerBody()?['data']?['LocationPipingMarked']` |
| Facility Name | `triggerBody()?['data']?['FacilityName']` |
| Competent Person | `triggerBody()?['data']?['CompetentPerson']` |
| Soil Type | `triggerBody()?['data']?['SoilType']` |
| Excavation Depth | `triggerBody()?['data']?['ExcavationDepth']` |
| Excavation Width | `triggerBody()?['data']?['ExcavationWidth']` |
| Protective System Type | `triggerBody()?['data']?['ProtectiveSystemType']` |
| Status | `triggerBody()?['data']?['Status']` |

**Tip:** For the Yes/No column (Location/Piping Marked), the value must be a boolean (`true` or `false`). For the Choice column (Status), the value must match one of the defined choices exactly (e.g. `Active`, `Closed`). Ensure the client always sends valid values.
...
| ExcavationDescription | `item()?['ExcavationDescription']` |
| WorkOrder | `item()?['WorkOrder']` |
| LocationPipingMarked | `item()?['LocationPipingMarked']` |
| FacilityName | `item()?['FacilityName']` |
| CompetentPerson | `item()?['CompetentPerson']` |
| SoilType | `item()?['SoilType']` |
| ExcavationDepth | `item()?['ExcavationDepth']` |
| ExcavationWidth | `item()?['ExcavationWidth']` |
| ProtectiveSystemType | `item()?['ProtectiveSystemType']` |
| Status | `item()?['Status']?['Value']` |

**Note:** The Choice column (Status) returns an object like `{"Value": "Active"}`. Use `?['Value']` to extract the string. The Yes/No column (LocationPipingMarked) returns a boolean directly -- no `?['Value']` needed.
...
- Full update: `{ "actionType": "update", "id": "42", "data": { "ExcavationDescription": "...", "Status": "Active" } }`
...
- **Java:** `src/main/resources/application.properties` > `pa.flow.excavation-url`
...
2. Open the app, go to Excavation Permits
...
2. **Yes/No columns require boolean.** The `LocationPipingMarked` column expects `true` or `false` (boolean), not `"Yes"` or `"No"` (string). Ensure the client sends the correct type.
