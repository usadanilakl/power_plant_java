# Power Automate Flow Setup: Hot Work

Step-by-step guide to create the Hot Work flow in Power Automate.

---

## Step 1: SharePoint List "Hot Work Permits"

List already exists with PascalCase internal names (e.g. `MeterModel`). Verify these columns exist:
...
| Foreman | Foreman | Single line of text | |
| FireWatch | Fire Watch | Single line of text | |
| MeterModel | Meter Model | Single line of text | |
| MeterNum | Meter Number | Single line of text | |
| SpecialInstructions | Special Instructions | Multiple lines of text | Plain text |
...
```json
{
  ...
  "data": {
    ...
    "MeterNum": "MN-001",
    "WorkScope": "Welding on boiler pipe",
    "SpecialInstructions": "Keep fire extinguisher within reach",
    "LocationOfWork": "Boiler Room",
    "Status": "Active"
  },
  "attachments": [
    {
...
| Foreman | `triggerBody()?['data']?['Foreman']` |
| Fire Watch | `triggerBody()?['data']?['FireWatch']` |
| Meter Model | `triggerBody()?['data']?['MeterModel']` |
| Meter Number | `triggerBody()?['data']?['MeterNum']` |
| Special Instructions | `triggerBody()?['data']?['SpecialInstructions']` |
| Location Of Work | `triggerBody()?['data']?['LocationOfWork']` |
| Status | `triggerBody()?['data']?['Status']` |

**Tip:** For Choice columns (Status), the value must match one of the defined choices exactly (e.g. `Active`, `Closed`, `Archived`). Ensure the client always sends valid values.
...
**Note:** Choice columns return objects like `{"Value": "Active"}`. Use `?['Value']` to extract the string.
...
- **Java:** `src/main/resources/application.properties` > `pa.flow.hot-work-url`
...
curl -X POST "<your-flow-url>" \
  ...
  -d '{
    ...
    "data": {
      ...
      "FireWatch": "Test Fire Watch",
      "MeterModel": "MSA Altair 5X",
      "MeterNum": "MN-001",
      "WorkScope": "Test hot work scope",
      "SpecialInstructions": "Keep fire extinguisher within reach",
      "LocationOfWork": "Boiler Room",
      "Status": "Active"
    },
...
2. Open the app, go to Hot Work Permits
3. Fill out a form and submit
4. Check the browser console for `[PA V2] Submitting hotWork: create`
...
2. **Date as string.** The Date column is stored as Single line of text, not a SharePoint date column. This avoids timezone conversion issues. Send it as a plain string (e.g. `"2026-03-03"`).
