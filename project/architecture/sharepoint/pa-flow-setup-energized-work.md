# Power Automate Flow Setup: Energized Work Permit

Step-by-step guide to create the Energized Work Permit flow in Power Automate.

---

## Step 1: SharePoint List "Energized Work Permits"

List already exists with PascalCase internal names (e.g. `CircuitDescription`). Verify these columns exist:
...
| WorkOrder | Work Order | Single line of text | |
| CircuitDescription | Circuit Description | Single line of text | |
| WorkDescription | Work Description | Multiple lines of text | Plain text |
| Justification | Justification | Multiple lines of text | Plain text |
| Requester | Requester | Single line of text | |
| RequesterDate | Requester Date | Single line of text | |
| QualifiedPersonSignature | Qualified Person Signature | Single line of text | |
| QualifiedPersonDate | Qualified Person Date | Single line of text | |
| PlantManagerSignature | Plant Manager Signature | Single line of text | |
| PlantManagerDate | Plant Manager Date | Single line of text | |
| WorkCanBePerformedSafely | Work Can Be Performed Safely | Yes/No | Default: No |
...
```json
{
  ...
  "data": {
    ...
    "Date": "2026-03-03",
    "Time": "08:00",
    "LocationOfWork": "Switchgear Room",
    "IssuedTo": "JD",
    "WorkOrder": "WO-2026-0301",
    "CircuitDescription": "480V Bus 1A",
    "WorkDescription": "Replace circuit breaker on energized panel",
    "Justification": "Cannot de-energize without full plant shutdown",
    "Requester": "DK",
    "RequesterDate": "2026-03-03",
    "QualifiedPersonSignature": "JD",
    "QualifiedPersonDate": "2026-03-03",
    "PlantManagerSignature": "MR",
    "PlantManagerDate": "2026-03-03",
...
3. List Name: **Energized Work Permits**
...
| Work Order | `triggerBody()?['data']?['WorkOrder']` |
| Circuit Description | `triggerBody()?['data']?['CircuitDescription']` |
| Work Description | `triggerBody()?['data']?['WorkDescription']` |
| Justification | `triggerBody()?['data']?['Justification']` |
| Requester | `triggerBody()?['data']?['Requester']` |
| Requester Date | `triggerBody()?['data']?['RequesterDate']` |
| Qualified Person Signature | `triggerBody()?['data']?['QualifiedPersonSignature']` |
| Qualified Person Date | `triggerBody()?['data']?['QualifiedPersonDate']` |
| Plant Manager Signature | `triggerBody()?['data']?['PlantManagerSignature']` |
| Plant Manager Date | `triggerBody()?['data']?['PlantManagerDate']` |
| Work Can Be Performed Safely | `triggerBody()?['data']?['WorkCanBePerformedSafely']` |
| Status | `triggerBody()?['data']?['Status']` |

**Tip:** For the Yes/No column (Work Can Be Performed Safely), the value must be a boolean (`true` or `false`). For the Choice column (Status), the value must match one of the defined choices exactly (e.g. `Active`, `Closed`). Ensure the client always sends valid values.
...
2. If **Yes**:
   ...
   - Inside the loop, add **SharePoint > Add attachment**:
     ...
     - List Name: **Energized Work Permits**
...
| CircuitDescription | `item()?['CircuitDescription']` |
| WorkDescription | `item()?['WorkDescription']` |
| Justification | `item()?['Justification']` |
| Requester | `item()?['Requester']` |
| RequesterDate | `item()?['RequesterDate']` |
| QualifiedPersonSignature | `item()?['QualifiedPersonSignature']` |
| QualifiedPersonDate | `item()?['QualifiedPersonDate']` |
| PlantManagerSignature | `item()?['PlantManagerSignature']` |
| PlantManagerDate | `item()?['PlantManagerDate']` |
| WorkCanBePerformedSafely | `item()?['WorkCanBePerformedSafely']` |
| Status | `item()?['Status']?['Value']` |

**Note:** The Choice column (Status) returns an object like `{"Value": "Active"}`. Use `?['Value']` to extract the string. The Yes/No column (WorkCanBePerformedSafely) returns a boolean directly -- no `?['Value']` needed.
...
- Full update: `{ "actionType": "update", "id": "42", "data": { "WorkDescription": "...", "Status": "Active" } }`
...
- **Java:** `src/main/resources/application.properties` > `pa.flow.energized-work-url`
...
2. Open the app, go to Energized Work Permits
...
2. **Yes/No columns require boolean.** The `WorkCanBePerformedSafely` column expects `true` or `false` (boolean), not `"Yes"` or `"No"` (string). Ensure the client sends the correct type.
...
4. **Get items returns max 5000.** The default Top Count for "Get items" is 100. Increase it to 5000 if needed.
