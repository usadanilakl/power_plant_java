 ...
 * HEAT TRACE
 *******************************************************************************************************************************/
async function fillHtInfoWindow(point){
    let form = await buildFormFromObject(point); 
    let infoFrame = document.getElementById('infoFrameHeat-Trace');
    let infoContainer = document.getElementById('infoWindowHeat-Trace');
    if(infoContainer === null) newInfoWindow("Heat-Trace");
    if(infoFrame.classList.contains('hide')) infoFrame.classList.remove('hide');
    ...
    if(point.equipmentList){
        let list = eqPointDropdown(point.equipmentList);
        infoContainer.appendChild(list);        
    }

}

async function fillEqDropdownWindow(points){
    let infoFrame = document.getElementById('infoFrameEquipment');
    let infoContainer = document.getElementById('infoWindowEquipment');
    if(infoContainer === null) newInfoWindow("Equipment");
    if(infoFrame.classList.contains('hide')) infoFrame.classList.remove('hide');
    ...
    if(points){
        let list = eqPointDropdown(points);
        infoContainer.appendChild(list);        
    }

}

function eqPointDropdown(points){
    let list = document.createElement('ul');
    points.forEach(e=>{
        let item = document.createElement('li');
        item.style.width = '100%'
        ...
        else if(e.objectType==="Equipment" || e.objectType==="LotoPoint"){
            button.classList.add('smallBtn');
            button.classList.add('lime');
        } 
        else{
        ...
        if(e.objectType === "Equipment" || e.objectType==="LotoPoint") {
            let addButton = document.createElement('button');
            buttons.appendChild(addButton);
            addButton.classList.add('addButtons');
            addButton.classList.add('yellow');
