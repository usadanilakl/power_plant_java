...
class UtilService{
    static dbClick(element, oneClickFunc, twoClickFunc) {
        let counter = 0;
        const clickCounter = () => {
            counter++;
            if (counter === 1) {
    ...
    static toggleBoolean(booleanVariable){
    ...
    static async loadDataIntoContainer(container,data){
        let content = await data();
        container.innerHTML = content;
    }

    static firstLetterToUpperCase(sentence){
        let res = "";
    ...
    static copyToClipboard(inputElement,messageElement) {
    ...
    static async checkClipboardAndPaste() {
        try {
    ...
    static async checkClipboardAndPasteShort(inputElement) {
            let value;
            if (navigator.clipboard) {
                const clipboardText = await navigator.clipboard.readText();
                if (clipboardText) {
    ...
    static async pasteFromClipboardWithoutClearing(inputElement) {
        let value;
        if (navigator.clipboard) {
            const clipboardText = await navigator.clipboard.readText();
            if (clipboardText) {
    ...
    static async saveInClipboard(text) {
        if (navigator.clipboard) {
