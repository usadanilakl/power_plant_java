    ...
    static async buildFormFromObject(point){
        ...
        for(let e in point){
            ...
            if(isCat){
                ...
                input.addEventListener('keyup', () => FormService.filterOptions(input,options)); //to filter options as user types into input field
                ...
                if(ModeService.MODES.editMode.state){
            ...
            input.addEventListener('click',async ()=>{
                let clipBoardText = await UtilService.checkClipboardAndPasteShort(input);
                if(!isCat){
