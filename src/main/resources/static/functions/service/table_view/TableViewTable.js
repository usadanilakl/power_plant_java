...
async function createTableWithFunctionFromObjects(objects, ignoreFields,action){
    console.log(JSON.stringify(ignoreFields))
    filteredArray = objects;
    console.log(filteredArray.length);
    let table = document.createElement('table');
    ...
    for(let e in objects[0]){
        ...
        search.id = `${e}-input`;
        search.addEventListener('change', filterTable);
        ...
        let key = e==='generalLocation' ? 'location' : e;
        
        if(await isCategory(key)){
            
            let options = allValues.filter(v=>v.category.alias===key).map(v=>v.name).sort();
            let optionContainer = document.createElement('div');
            th.appendChild(optionContainer);
            optionContainer.classList.add('searchable-dropdown-content');
            optionContainer.id = `${e}-options`;
            ...
            search.addEventListener('keyup', () => filterFunction(e));
            ...
            let detectClickOutsideOfDropdown = function(event) {
                var isClickInside;
                if(search && optionContainer) isClickInside = search.contains(event.target) || optionContainer.contains(event.target);
                if (!isClickInside) {
                    document.getElementById(`${e}-options`).classList.remove("show");
            ...
            optionContainer.addEventListener('click', function(event) {
                if (event.target.tagName.toLowerCase() === 'div') {
                    document.getElementById(`${e}-input`).value = event.target.textContent;
                    document.getElementById(`${e}-options`).classList.remove("show");
                    filterTable();
                }
            });
