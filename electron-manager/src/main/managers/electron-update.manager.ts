...
export class ElectronUpdateManager {
  ...
  public applyUpdate(): IpcResult {
    ...
    const mainLines = [
      ...
      '',
      'REM Verify ZIP still exists. The app has already exited (we waited on its PID above), and NO swap has',
      'REM happened yet, so if we bail here we must RELAUNCH the existing install — never park an unattended',
      'REM PC at a console prompt with no app (same fix as the extraction-failure path below).',
      `if not exist "${zipPath}" (`,
      '    echo [%date% %time%] ERROR: ZIP not found - relaunching existing version >> "%LOGFILE%"',
      '    echo.',
      '    echo ERROR: Update ZIP not found - the existing version is being restarted.',
      `    echo   ${zipPath}`,
      '    echo Log: "%LOGFILE%"',
      `    start "" "${exePath}"`,
      '    timeout /t 20 /nobreak >nul',
