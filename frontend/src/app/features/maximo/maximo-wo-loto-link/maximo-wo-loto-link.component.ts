...
import { MaximoWoLotoLinkService, LotoLink } from '../maximo-wo-loto-link.service';

/** WO→LOTO linking for ANY work order (desktop): linked LOTOs + attach a current (not-closed) LOTO. */
@Component({
  ...
  `,
  styles: [`
    .wl { border: 1px solid var(--border-color); border-radius: 10px; padding: 0.6rem 0.7rem; margin: 0.6rem 0; }
    .wl-hd { font-weight: 700; font-size: 0.9rem; margin-bottom: 0.4rem; }
    .wl-msg, .wl-none { color: var(--secondary-text, #888); font-size: 0.85rem; }
    .wl-row { display: flex; align-items: center; gap: 0.6rem; padding: 0.35rem 0; border-bottom: 1px solid var(--border-color); }
    ...
    .wl-x { margin-left: auto; background: transparent; border: 1px solid #ef5350; color: #ef5350; border-radius: 6px; padding: 0.2rem 0.55rem; font-size: 0.75rem; font-weight: 700; cursor: pointer; }
    .wl-add { margin-top: 0.5rem; background: transparent; border: 1px solid #7E57C2; color: #7E57C2; border-radius: 8px; padding: 0.45rem 0.8rem; font-weight: 700; font-size: 0.82rem; cursor: pointer; }
    ...
    .wl-pick { max-height: 240px; overflow-y: auto; display: flex; flex-direction: column; gap: 0.25rem; }
...
export class MaximoWoLotoLinkComponent implements OnInit {
  ...
  private api = inject(MaximoWoLotoLinkService);
