...
import { MaximoWoLotoLinkService } from '../../maximo/maximo-wo-loto-link.service';
...
/** LOTO→WO linking (desktop): a "Linked Work Orders" panel on a LOTO permit. Search Maximo WOs — All / Outage /
 *  TOI-TMOD — and attach one. The link lives on the LOTO ({@code linkedWonums}). */
@Component({
  ...
  `,
  styles: [`
    .lw { border: 1px solid var(--border-color); border-radius: 10px; padding: 0.6rem 0.7rem; margin: 0.6rem 0; background: var(--secondary-background); }
    .lw-hd { font-weight: 700; font-size: 0.9rem; margin-bottom: 0.4rem; }
    .lw-msg, .lw-none { color: var(--secondary-text, #888); font-size: 0.85rem; }
    .lw-row { display: flex; align-items: center; gap: 0.6rem; padding: 0.35rem 0; border-bottom: 1px solid var(--border-color); }
    .lw-wo { font-weight: 700; font-size: 0.85rem; }
    .lw-x { margin-left: auto; background: transparent; border: 1px solid #ef5350; color: #ef5350; border-radius: 6px; padding: 0.2rem 0.55rem; font-size: 0.75rem; font-weight: 700; cursor: pointer; }
    .lw-add { margin-top: 0.5rem; background: transparent; border: 1px solid #7E57C2; color: #7E57C2; border-radius: 8px; padding: 0.45rem 0.8rem; font-weight: 700; font-size: 0.82rem; cursor: pointer; }
    ...
    .lw-f.on { background: var(--accent-color, #26C6DA); border-color: var(--accent-color, #26C6DA); color: #fff; }
...
export class LotoWoLinkComponent implements OnInit {
  ...
  private lotoApi = inject(MaximoWoLotoLinkService);
  ...
  private runSearch(): void {
    ...
    if (f === 'outage') { this.mx.listOutageWorkOrders().subscribe({ next: l => done(this.clientFilter(l, q)), error: fail }); }
      ...
      const c = { wonumContains: byNum ? q : '', textContains: byNum ? '' : q } as MaximoWorkOrderCriteria;
      this.mx.listWorkOrdersByCriteria(c, 40).subscribe({ next: done, error: fail });
    }
  }
