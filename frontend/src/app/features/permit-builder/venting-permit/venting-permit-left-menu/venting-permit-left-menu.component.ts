...
@Component({
  selector: 'app-venting-permit-left-menu',
  standalone: true,
  imports: [CommonModule, RfToggleMenuComponent],
  templateUrl: './venting-permit-left-menu.component.html',
  styleUrl: './venting-permit-left-menu.component.css',
})
export class VentingPermitLeftMenuComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.currentVentingPermitService.allActivePermits$.pipe(
  ...
  private regroup(items: any[]): void {
    const grouped = this.permitMenuService.groupPermits(items, this.groupBy(), 'requestingIndividual');
    this.menuItems.set(grouped);
  }

  onItemClick(item: NestedItem): void {
    if (item.values && item.values.length > 0) return;
    this.currentVentingPermitService.setCurrentPermit(Number(item.id));
  }
