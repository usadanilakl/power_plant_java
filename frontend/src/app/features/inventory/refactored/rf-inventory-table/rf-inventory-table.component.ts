...
import { RfInventoryTableClickService } from '../services/rf-inventory-table-click.service';
...
@Component({
  selector: 'app-rf-inventory-table',
  standalone: true,
  ...
  providers: [
    ...
    TableResizeService,
    TableSyncService,
    { provide: TableClickService, useClass: RfInventoryTableClickService },
  ...
  styles: [`
    ...
    .table-wrapper { flex: 1 1 auto; display: flex; flex-direction: column;
      min-height: 0; height: 100%; overflow: hidden; }
    .cell-thumbnail { width: 36px; height: 36px; object-fit: cover; border-radius: 4px; cursor: pointer;
      border: 1px solid var(--border-color); transition: transform 0.15s; }
    ...
    .lightbox-nav { position: fixed; top: 50%; background: rgba(255,255,255,0.15); border: none;
      color: white; font-size: 28px; cursor: pointer; padding: 12px 16px;
      border-radius: 4px; transform: translateY(-50%); }
    .lightbox-nav:hover { background: rgba(255,255,255,0.3); }
    .lightbox-prev { left: 16px; }
    .lightbox-next { right: 16px; }
    .lightbox-counter { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%);
      color: rgba(255,255,255,0.8); font-size: 14px; }
  `]
})
export class RfInventoryTableComponent implements AfterViewInit {
  private stateService = inject(RfInventoryStateService);
  private apiService = inject(RfInventoryApiService);

  selectedItemsEvent = output<InventoryItemDto[]>();

  imageTpl = viewChild.required<TemplateRef<any>>('imageTpl');
  actionsTpl = viewChild.required<TemplateRef<any>>('actionsTpl');

  private items$ = toSignal(this.stateService.allItems$, { initialValue: [] as InventoryItemDto[] });
  ...
  ngAfterViewInit(): void {
    const baseColumns = InventoryItemDto.toTableColumns();
  ...
  isLoadingImages(itemId: number): boolean { return this.loadingSet.has(itemId); }
  ...
  onThumbnailClick(event: MouseEvent, item: InventoryItemDto): void {
  ...
  onViewDetails(event: MouseEvent, item: InventoryItemDto): void {
    event.stopPropagation();
    this.stateService.openDetail(item);
  }
}
