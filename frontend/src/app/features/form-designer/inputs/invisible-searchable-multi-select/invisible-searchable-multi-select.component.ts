...
export class InvisibleSearchableMultiSelectComponent implements ControlValueAccessor, OnDestroy {
  ...
  private filterAndSortOptions(searchTerm: string = ''): void {
    ...
    if (searchTerm) {
      filtered = this._options.filter(option =>
        option.label.toLowerCase().includes(searchTerm)
      );
    }

    this.filteredAndSortedOptions = filtered.sort((a, b) => {
      const aIsSelected = this.values.includes(a.value);
      const bIsSelected = this.values.includes(b.value);
  ...
  toggleOption(option: Option, event: MouseEvent): void {
    ...
    this.filterAndSortOptions(); // Re-sort to keep selected at top
  }
  ...
  onEditOption(event: MouseEvent): void {
    event.stopPropagation();
    // For multi-select, it's ambiguous which to edit.
    // You might want to pass the last selected or handle this differently.
    // Emitting without a value for now.
    this.edit.emit();
    this.closeDropdown();
  }
}