...
import { RfSdsTableClickService } from '../services/rf-sds-table-click.service';
...
@Component({
  ...
  providers: [
    ...
    TableResizeService,
    TableSyncService,
    { provide: TableClickService, useClass: RfSdsTableClickService },
...
export class RfSdsTableComponent implements AfterViewInit {
  private stateService = inject(RfSdsStateService);
  private apiService = inject(RfSdsApiService);

  selectedItemsEvent = output<SdsChemicalDto[]>();
  ...
  ngAfterViewInit(): void {
    const baseColumns = SdsChemicalDto.toTableColumns();
  ...
  onThumbnailClick(event: MouseEvent, item: SdsChemicalDto): void {
  ...
  onViewDetails(event: MouseEvent, item: SdsChemicalDto): void {
    event.stopPropagation();
    this.stateService.openDetail(item);
  }
}
