...
export interface LotoBoxModel extends BaseModel {
  number: number;
  loto: LotoDto;
  lotoAccessoryStatus: ValueDto;
}

export class LotoBoxDto extends BaseDto implements LotoBoxModel {
  number: number;
  loto: LotoDto;
  lotoAccessoryStatus: ValueDto;

  constructor(data: Partial<LotoBoxModel> = {}) {
    super(data);
  ...
  static override fromJson(json: any): LotoBoxDto {
    if (!json) {
