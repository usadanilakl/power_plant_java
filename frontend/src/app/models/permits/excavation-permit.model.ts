...
export class ExcavationPermitDto extends BaseDto implements ExcavationPermitModel {
  ...
  static toFormFields(
    fields: ExcavationPermitFieldName[] = ['date', 'time', 'location', 'issuedTo', 'workScope']
  ): FormField[] {
    const allFields: { [key in ExcavationPermitFieldName]?: FormField } = {
  ...
  static toTableColumns(
    fields: ExcavationPermitFieldName[] = ['id', 'permitNumber', 'date', 'location', 'issuedTo', 'workScope']
  ): Column[] {
