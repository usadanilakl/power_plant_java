...
/**
 * TOI/TMOD (Temporary Operation Instruction / Temporary Modification) — Active/Closed tabs of records backed by
 * Maximo WOs (marker in the description). Create a record (WAPPR WO + risk assessment/instruction form written as
 * the first log note), add log notes while active, and close it (who + comments). Full WO detail reuses the WO sheet.
 */
@Component({
  selector: 'app-maximo-toi-page',
  standalone: true,
  imports: [MainLayoutComponent, DatePipe, MaximoTreePickerComponent, MaximoWoDetailComponent],
  template: `
    <app-main-layout [header]="'TOI / TMOD'">
      <ng-container main-content>
        <div class="ti">
          <div class="ti-top">
            <button class="ti-back" (click)="back()">← Maximo</button>
            @if (mode() === 'list') {
              <button class="ti-new" (click)="startCreate()">+ New</button>
            } @else {
              <button class="ti-refresh" (click)="mode.set('list')">← List</button>
            }
          </div>
          ...
          @if (mode() === 'list') {
            <div class="ti-tabs">
              ...
              <button class="ti-refresh sm" [disabled]="loading()" (click)="load()">{{ loading() ? '…' : '↻' }}</button>
            </div>
                ...
                <div class="ti-card">
                  <button class="ti-row" (click)="toggle(wo)">
                    <span class="ti-main">
                      ...
                      <span class="ti-sub">{{ wo.wonum }} · {{ wo.status }}@if (wo.location) { · 📍 {{ wo.location }} }</span>
                    </span>
                  ...
                  @if (expanded() === wo.href) {
                    <div class="ti-body">
                      <button class="ti-detail-btn" (click)="detailWo.set(wo)">🗂 Full WO details — attachments · dates · tasks</button>
                      @if (!isClosed(wo)) { <button class="ti-edit-btn" (click)="startEdit(wo)">✎ Edit title / location / asset</button> }
                      ...
                      @if (!isClosed(wo)) {
                        <textarea class="ti-ta" rows="2" placeholder="Add a log note…"
                                  [value]="noteDraft()[wo.href] || ''" (input)="setNoteDraft(wo.href, $any($event.target).value)"></textarea>
                        ...
                        <p class="ti-closed-note">✓ Closed</p>
                      }
                    </div>
                  }
                </div>
          ...
          @if (mode() === 'create') {
            ...
            <label class="ti-f">Title <span class="req">*</span>
              <input type="text" [value]="cTitle()" (input)="cTitle.set($any($event.target).value)" placeholder="e.g. U1 & U2 Start up / Shutdown / trip / spin cool">
            </label>

            <div class="ti-fh">Location / asset</div>
            <app-maximo-tree-picker mode="both" [assetnum]="cAsset()" [location]="cLocation()"
                                    (selection)="onTreePick($event)"></app-maximo-tree-picker>
            @if (cLocation() || cAsset()) { <p class="ti-picked">{{ cAsset() ? 'Asset ' + cAsset() : '' }}{{ cAsset() && cLocation() ? ' · ' : '' }}{{ cLocation() ? 'Loc ' + cLocation() : '' }}</p> }

            <div class="ti-row2">
              <label class="ti-f">Work type
                <input type="text" [value]="cWorktype()" (input)="cWorktype.set($any($event.target).value)" placeholder="optional">
              </label>
              <label class="ti-f">Originator
                <input type="text" [value]="cOriginator()" (input)="cOriginator.set($any($event.target).value)">
              </label>
            </div>
            ...
            @for (s of sections; track s.key) {
              <div class="ti-risk">
                ...
                <select (change)="setConsequence(s.key, $any($event.target).value)">
                  ...
                  @for (o of consequences(s.key); track o.label; let i = $index) {
                    <option [value]="i" [selected]="risk()[s.key].c?.label === o.label">{{ o.label }} ({{ o.pts }})</option>
                  }
                </select>
                <select (change)="setProbability(s.key, $any($event.target).value)">
                  <option value="">Probability…</option>
                  @for (o of probabilities; track o.label; let i = $index) {
                    <option [value]="i" [selected]="risk()[s.key].p?.label === o.label">{{ o.label }} ({{ o.pts }})</option>
                  }
                </select>
              </div>
            }
            <div class="ti-total" [class]="levelClass()">Total: {{ riskTotal() }} — {{ level() }}</div>

            <label class="ti-f">TOI/TMOD Instructions
              <textarea rows="4" [value]="cInstructions()" (input)="cInstructions.set($any($event.target).value)"></textarea>
            </label>
            <label class="ti-f">Risk Identified
              <textarea rows="2" [value]="cRiskIdentified()" (input)="cRiskIdentified.set($any($event.target).value)"></textarea>
            </label>
            <label class="ti-f">Countermeasures / Controls
              <textarea rows="2" [value]="cCountermeasures()" (input)="cCountermeasures.set($any($event.target).value)"></textarea>
            </label>
            <div class="ti-row2">
              <label class="ti-f">Approved by
                <input type="text" [value]="cApprovedBy()" (input)="cApprovedBy.set($any($event.target).value)">
              </label>
              <label class="ti-f">Approved date
                <input type="date" [value]="cApprovedDate()" (input)="cApprovedDate.set($any($event.target).value)">
              </label>
            </div>
            <label class="ti-f">Expected completion
              <input type="date" [value]="cExpectedCompletion()" (input)="cExpectedCompletion.set($any($event.target).value)">
            </label>
        ...
        @if (editing(); as wo) {
          <div class="ti-backdrop" (click)="editing.set(null)">
            <div class="ti-modal" (click)="$event.stopPropagation()">
              ...
              <label class="ti-f">Title <span class="req">*</span>
                <input type="text" [value]="eTitle()" (input)="eTitle.set($any($event.target).value)">
              </label>
              <div class="ti-fh">Location / asset — equipment sets both, location only sets location</div>
              <app-maximo-tree-picker mode="both" [assetnum]="eAsset()" [location]="eLocation()" (selection)="onEditTreePick($event)"></app-maximo-tree-picker>
              @if (eLocation() || eAsset()) { <p class="ti-picked">{{ eAsset() ? 'Asset ' + eAsset() : '' }}{{ eAsset() && eLocation() ? ' · ' : '' }}{{ eLocation() ? 'Loc ' + eLocation() : '' }}</p> }
              <label class="ti-f">Work type <input type="text" [value]="eWorktype()" (input)="eWorktype.set($any($event.target).value)"></label>
              <label class="ti-f">Instructions <textarea rows="4" [value]="eInstructions()" (input)="eInstructions.set($any($event.target).value)"></textarea></label>
              <div class="ti-actions">
                <button class="ti-add" [disabled]="savingEdit() || !eTitle().trim()" (click)="submitEdit(wo)">{{ savingEdit() ? 'Saving…' : 'Save' }}</button>
                <button class="ti-cancel" (click)="editing.set(null)">Cancel</button>
              </div>
            </div>
          </div>
        }
        ...
        @if (closing(); as wo) {
          <div class="ti-backdrop" (click)="closing.set(null)">
            <div class="ti-modal" (click)="$event.stopPropagation()">
              ...
              <label class="ti-f">Closed by <span class="req">*</span>
                <input type="text" [value]="closeBy()" (input)="closeBy.set($any($event.target).value)">
              </label>
              <label class="ti-f">Comments
                <textarea rows="3" [value]="closeComments()" (input)="closeComments.set($any($event.target).value)"></textarea>
              </label>
              <label class="ti-f">Attach documents (optional)
                <input type="file" multiple (change)="onCloseFiles($event)">
              </label>
              ...
              <div class="ti-actions">
                ...
                <button class="ti-cancel" (click)="closing.set(null)">Cancel</button>
              </div>
            </div>
          </div>
        }
  ...
  `,
  styles: [`
    .ti { padding: 0.5rem 0.75rem 2rem; }
    .ti-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.4rem; }
    .ti-back, .ti-refresh { background: transparent; border: none; color: var(--accent-color); font-weight: 700; cursor: pointer; }
    .ti-new { background: #27ae60; color: #fff; border: none; border-radius: 8px; padding: 0.4rem 0.8rem; font-weight: 700; cursor: pointer; }
    .ti-tabs { display: flex; gap: 0.4rem; align-items: center; margin-bottom: 0.6rem; }
    .ti-tab { flex: 1; background: transparent; border: 1px solid var(--border-color); color: var(--secondary-text, #888); border-radius: 8px; padding: 0.45rem; font-weight: 700; cursor: pointer; }
    .ti-tab.active { background: var(--accent-color); border-color: var(--accent-color); color: #fff; }
    .ti-refresh.sm { flex: 0 0 auto; border: 1px solid var(--border-color); border-radius: 8px; padding: 0.45rem 0.6rem; color: var(--primary-text); }
    .ti-err { background: rgba(239,83,80,0.12); border: 1px solid #ef5350; border-radius: 8px; padding: 0.5rem 0.6rem; color: var(--primary-text); font-size: 0.85rem; }
    .ti-msg, .ti-none { color: var(--secondary-text, #888); font-size: 0.85rem; }
    .ti-card { border: 1px solid var(--border-color); border-radius: 12px; overflow: hidden; background: var(--secondary-background); margin-bottom: 0.5rem; }
    .ti-row { width: 100%; display: flex; align-items: center; gap: 0.6rem; text-align: left; background: transparent; border: none; color: var(--primary-text); padding: 0.6rem 0.7rem; cursor: pointer; font-family: inherit; }
    .ti-main { flex: 1; display: flex; flex-direction: column; gap: 0.15rem; min-width: 0; }
    .ti-title { font-weight: 700; font-size: 0.92rem; }
    .ti-sub { font-size: 0.75rem; color: var(--secondary-text, #888); }
    .ti-caret { color: var(--secondary-text, #888); }
    .ti-body { padding: 0.3rem 0.8rem 0.8rem; border-top: 1px solid var(--border-color); }
    .ti-detail-btn { width: 100%; margin: 0.5rem 0; background: transparent; border: 1px solid var(--accent-color); color: var(--accent-color); border-radius: 8px; padding: 0.5rem; font-weight: 700; font-size: 0.85rem; cursor: pointer; }
    .ti-edit-btn { width: 100%; margin-bottom: 0.5rem; background: transparent; border: 1px solid #7E57C2; color: #7E57C2; border-radius: 8px; padding: 0.5rem; font-weight: 700; font-size: 0.85rem; cursor: pointer; }
    .ti-h { margin: 0.5rem 0 0.4rem; font-size: 0.9rem; }
    ...
    .ti-note.rec { border-left-color: #42A5F5; }
    .ti-note.cls { border-left-color: #EC407A; }
    .ti-note-hd { font-weight: 700; font-size: 0.82rem; }
    .ti-note-txt { white-space: pre-wrap; font-size: 0.85rem; margin-top: 0.2rem; }
    .ti-note-meta { font-size: 0.7rem; color: var(--secondary-text, #888); margin-top: 0.2rem; }
    .ti-ta, .ti-f input, .ti-f textarea, .ti-risk select { width: 100%; box-sizing: border-box; border: 1px solid var(--border-color); border-radius: 8px; padding: 0.5rem; font-family: inherit; font-size: 0.9rem; background: var(--card-bg, var(--secondary-background)); color: var(--primary-text); }
    .ti-actions { display: flex; gap: 0.5rem; margin-top: 0.4rem; }
    .ti-add { background: var(--accent-color); color: #fff; border: none; border-radius: 8px; padding: 0.5rem 0.8rem; font-weight: 700; cursor: pointer; }
    .ti-close-btn, .ti-cancel { background: transparent; border: 1px solid var(--border-color); color: var(--primary-text); border-radius: 8px; padding: 0.5rem 0.8rem; font-weight: 700; cursor: pointer; }
    .ti-close-btn { border-color: #EC407A; color: #EC407A; }
    .ti-closed-note { color: #EC407A; font-weight: 700; font-size: 0.85rem; }
    .ti-formh { font-size: 1rem; margin: 0.4rem 0; }
    .ti-f { display: block; font-size: 0.8rem; color: var(--secondary-text, #888); margin-bottom: 0.6rem; }
    .ti-f input, .ti-f textarea { margin-top: 0.2rem; color: var(--primary-text); }
    .req { color: #ef5350; }
    .ti-fh { font-size: 0.8rem; color: var(--secondary-text, #888); margin-bottom: 0.3rem; }
    .ti-picked { font-size: 0.78rem; color: var(--accent-color); margin: 0.2rem 0 0.6rem; }
    .ti-hint { font-size: 0.75rem; color: var(--secondary-text, #888); margin: 0.2rem 0 0.5rem; }
    .ti-row2 { display: flex; gap: 0.5rem; } .ti-row2 .ti-f { flex: 1; }
    .ti-risk { border: 1px solid var(--border-color); border-radius: 8px; padding: 0.5rem; margin-bottom: 0.5rem; }
    .ti-risk-hd { display: flex; justify-content: space-between; font-weight: 700; font-size: 0.85rem; margin-bottom: 0.4rem; }
    .ti-risk-score { background: var(--secondary-background); border: 1px solid var(--border-color); border-radius: 6px; padding: 0 0.4rem; }
    .ti-risk select { margin-bottom: 0.35rem; }
    .ti-total { text-align: center; font-weight: 800; padding: 0.5rem; border-radius: 8px; margin-bottom: 0.8rem; }
    ...
    .ti-submit { width: 100%; background: #27ae60; color: #fff; border: none; border-radius: 8px; padding: 0.7rem; font-weight: 700; font-size: 0.95rem; cursor: pointer; }
    ...
    .ti-modal { background: var(--secondary-background); border-radius: 12px; padding: 1rem; width: 100%; max-width: 460px; }
  `]
})
export class MaximoToiPageComponent implements OnInit {
  ...
  closeBy = signal('');
  closeComments = signal('');
  closeFiles = signal<File[]>([]);
  ...
  load(): void {
    ...
    this.api.listTois().subscribe({
      ...
      error: () => { this.error.set('Couldn’t load TOI/TMOD records — Maximo may be unreachable. Tap ↻ to retry.'); this.loading.set(false); }
    });
  }
  ...
  submitClose(wo: MaximoWorkOrder): void {
    ...
    // Attach the documents first (while the WO is still editable), THEN close (status → COMP).
    const uploads = files.length ? forkJoin(files.map(f => this.api.uploadWoAttachment(wo.href, f))) : of([]);
