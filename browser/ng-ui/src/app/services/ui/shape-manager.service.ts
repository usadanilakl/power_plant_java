...
import { Shape, RectangleShape, ImageShape, SVGSymbolShape } from '../../models/ui/shape.model';
...
export class ShapeManagerService {
  ...
  // State signals
  private shapesSignal = signal<Shape[]>(this.testShapes);
  ...
  readonly singleSelectedShapeId = this.singleSelectedShapeIdSignal.asReadonly();
  ...
  addShapes(newShapes: Shape[]): void {
  ...
  replaceShape(shapeId: number, newShape: Shape): void {
    this.shapesSignal.update(shapes =>
  ...
  // Selection Management
    ...
    this.shapesSignal.update(shapes =>
        shapes.map(shape => {
        if (shape.id !== shapeId) return shape;
        return { ...shape, isSelected: true } as Shape;
        })
    );
    }
    ...
    this.shapesSignal.update(shapes =>
        shapes.map(shape => {
        if (shape.id !== shapeId) return shape;
        return { ...shape, isSelected: false } as Shape;
        })
    );
    }
